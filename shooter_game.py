...
from time import time as timer #функция для засекания времени, даем другое название
...
class Player(GameSprite):
    #метод управления спрайтом стрелками клавиатуры
    def update(self):
...
rel_time = False #флаг, огтвечающий за перезарядку
...
while run:
   ...
   if not finish:
       ...
       #проверка столкновения пули и монстров(и пуля и монстр исчезают)
       collides = sprite.groupcollide(monsters, bullets, True, True)
       for c in collides:
           score = score + 1
           monster = Enemy(img_enemy, randint(80, win_width - 80), -40, 80, 50, randint(1, 5))
           monsters.add(monster)
       #умешение кол-ва жизней при столкновением с врагом
